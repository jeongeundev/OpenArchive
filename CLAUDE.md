...
- CRITICAL: **벡터 정렬(`ORDER BY embedding <=> ...`)이 있는 서브쿼리에 `documents` JOIN을 넣지 마라.** 조인이 있으면 플래너가 조인을 먼저 수행해 HNSW 인덱스를 못 쓴다 — `enable_seqscan=off`로도 복구되지 않는다(강제 하 실측 255ms vs 106ms). 권한 필터·`DISTINCT ON`은 후보를 확보한 **다음 단계**에서 적용한다 (ADR-018 개정). 단 **수치는 전부 `enable_seqscan=off` 조건이며, 강제 없이 HNSW를 타는 규모는 미확인이다** (`OPENSQL_RESEARCH.md` §12 16번).
