...
- CRITICAL: **후보 `LIMIT`은 `hnsw.ef_search` 예산에서 역산해 그보다 작게 둔다.** 등호에서도 모자란다(`ef_search=200`, `LIMIT 200` → 193행). 배수를 상수로 박지 말고 `(EF_SEARCH - 1) // MAX_K`처럼 계산하며, `random_page_cost=1.1`에서는 400부터 Seq Scan으로 떨어지므로 `SET LOCAL` 값은 아래·위 벽 사이의 200을 유지한다 (ADR-011 보강 4, `OPENSQL_RESEARCH.md` §12-22).
