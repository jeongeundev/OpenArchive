# Step 0: search-symmetric

## 배경 — edge 저장이 단방향으로 바뀐다. 읽는 쪽을 먼저 대칭으로 만든다

지금(`backend/migrations/008_edges_triggers.sql`) `document_edges`는 A가 계산한 관계를
`(A→B)`·`(B→A)` 두 행으로 넣고, 재임베딩 때 `src=NEW OR dst=NEW`를 전부 지운다. 그래서 문서
하나를 재임베딩하면 남이 발견한 관계까지 사라진다(#93 R4). 이 phase의 step 2
(`014_edges_triggers.sql`)가 저장을 **단방향**으로 바꾼다 — A가 계산하면 `(A→B)` 하나, B가
계산하면 `(B→A)` 하나이며 각자 자기 행만 교체한다.

저장이 단방향이 되면 관계를 **읽는 쪽이 양방향으로 읽어야** 한다. 조회 대칭은 ADR-029 개정의
일부다 — "관계는 무방향이고, 어느 쪽이 발견했든 이어진다".

**순서가 중요하다.** 조회 측 테스트(`test_search.py`·`test_related.py`·`test_mcp_server.py`·
`test_visibility.py` 등 17개)는 "source를 먼저 넣고 후보를 나중에 넣은 뒤 source의 관계를 읽는"
픽스처다. 저장을 먼저 바꾸면 `(후보→source)` 행만 남아 `src = source`로 읽는 조회가 전부 비고
17개가 깨진다(실측). 그래서 **읽기를 먼저 대칭으로 바꾼다** — 양방향 저장(008) 위에서는 같은
쌍이 두 번 나오므로 접어서 한 번만 내야 하고, 단방향 저장(014) 위에서는 역방향 행이 그대로
이웃이 된다. 두 저장 형식 모두에서 같은 결과를 내는 읽기가 이 step의 산출물이다.

이 step은 `backend/app/services/search.py`의 그래프 순회만 고친다. 관련 문서·태그 추천
(`related.py`)은 step 1이다. `clusters.py`는 이미 쌍으로 접어 읽고 `diagnostics.py`는 이미
...
- `backend/migrations/008_edges_triggers.sql` — 현재의 양방향 저장(`both_directions` CTE). step 2가 이를 단방향으로 바꾼다
...
바꾸지 않는다. 주석에 ADR-029 개정을 가리켜 둔다(014는 아직 없다 — 파일 번호를 적지 마라).

지금 DB는 양방향 저장이므로 이 변경은 기존 결과를 바꾸지 않아야 한다 — 같은 쌍이 두 번 나오는
것을 `walk_targets`의 `DISTINCT ON`이 접는지가 테스트 3이다.
...
- `related.py`를 고치지 마라. 이유: step 1의 일이다.
- 마이그레이션을 만들지 마라. 이유: 저장 형식 변경은 step 2의 일이다. 이 step은 두 저장 형식
  모두에서 같은 결과를 내는 읽기만 만든다.
- 순회 상한(노드당 edge 수·kind 제한)을 추가하지 마라. 이유: 문서당 5건 상한(step 2)이 밀도를
