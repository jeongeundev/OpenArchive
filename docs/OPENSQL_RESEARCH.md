...
**시나리오 ① PostgreSQL `SIGKILL` — 감지 46 ms, 접속 재개 5.85초** (1표본 — 대표값이 아니다. 아래 재측정 참조)
...
**시나리오 ①-재측정 — `46 ms`는 대표값이 아니다** [실측 2026-08-22]

제출 서류에 쓸 수치를 확정하려고 `scripts/demo_recovery.sh`로 postmaster SIGKILL을 4회 더
주입했다. 여기에 같은 날 실사용 검증에서 얻은 1표본(⓪)을 합쳐 6표본을 본다.
**감지 시각이 표본마다 크게 달랐고, 서비스 재개도 9초 넘게 벌어졌다.**

| 표본 | t2 감지 | t3 재기동 시작 | t4 앱 재접속(6432) | t5 잡 0 | t6 정합성 0 |
|---|---|---|---|---|---|
| 2026-08-09 | **0.046 s** | 0.233 s | (DB 로그 기준 5.85 s) | 미측정 | 미측정 |
| 2026-08-22 ⓪ | 5.60 s | 5.88 s | **22.38 s** | 25.87 s | 26.06 s |
| 2026-08-22 ① | 1.942 s | 2.136 s | 13.02 s | 16.48 s | 16.65 s |
| 2026-08-22 ② | 8.472 s | 8.683 s | 19.17 s | 22.76 s | 22.96 s |
| 2026-08-22 ③ | 5.694 s | 5.894 s | 13.17 s | 16.66 s | 16.86 s |
| 2026-08-22 ④ | 5.604 s | 5.818 s | 13.37 s | 16.84 s | 17.01 s |

**감지는 `loop_wait=10`초 주기 안에서 사실상 균등하게 흩어진다.** 0.046 s는 6표본 중 가장 운이
좋았던 값이지 대표값이 아니다. **`46 ms`를 단독으로 인용하면 안 된다** — 심사자가 재현하면
초 단위가 나올 확률이 훨씬 높다.
...
| 구간 | 범위 | 성격 |
|---|---|---|
| 감지 (t2) | 0.05 ~ 8.5 s | `loop_wait=10`에 좌우되는 **주기 대기**. 재현하면 이 폭 안 아무 값이나 나온다 |
| 재기동 (t3 → t4) | 7.3 ~ 16.5 s | 옛 인스턴스 정리 · 기동 · 앱 재접속 폴링 |
| **서비스 재개 (t4)** | **13.0 ~ 22.4 s** (5표본) | 앱이 OpenProxy 경유로 다시 쿼리에 성공한 시점 |
| 잡 재개·수렴 (t4 → t6) | +3.5 s (5표본 3.46~3.59 s) | 워커 폴링 5초 주기와 정합. 구간 중 가장 안정적 |

> **5.85 s와 22 s대는 모순이 아니라 경계가 다르다.** 5.85 s는 PostgreSQL 로그가 `이제
> 데이터베이스 서버로 접속할 수 있습니다`를 찍은 **DB 경계**이고, t4·t5는 앱이 OpenProxy를
> 거쳐 실제로 쿼리에 성공한 **서비스 경계**다. 사용자와 심사자가 체감하는 값은 후자다.

**측정 조건**: `EMBEDDING_PROVIDER=fake`. t5·t6은 모델 추론을 포함하지 않으므로 잡 재개
지연의 **하한**이다. t2·t3·t4는 프로바이더와 무관하다. 전 회차에서 `timeline`은 1로 유지되고
role은 primary였다 — 승격은 일어나지 않았다.

**운영 메모**: 복구 직후에는 검증 쿼리가 타임아웃해 스크립트가 사전 점검이나 마지막 정리
단계에서 실패한다. 연속 실행 시 **회차 사이에 60초 이상**을 두어야 한다. 시연 촬영은 단발
실행이므로 영향받지 않는다.
...
#### ⚠️ 재측정 (2026-08-11, m8 step 6 화면 검증 중) — 편향은 짧은 문서에 국한되지 않는다

**측정 조건.** 위 측정은 `FakeProvider` 벡터였다(`config.py`의 기본 프로바이더가 `fake`이며,
DB 벡터를 `FakeProvider`로 재현해 최대 오차 `7e-9`로 확인했다). 아래는 같은 코퍼스를
**실 `BAAI/bge-m3`로 재임베딩**한 63문서·280청크다(재현 오차 0.615로 fake가 아님을 확인).

진단 화면이 `score >= 0.95`를 "내용이 겹치는 문서 / 정리 후보"로 제시하는데, 그 목록이
실제로 무엇인지 확인했다.
...
**하한을 올려도 걸러지지 않는다.** 하한 4에서 남는 쌍의 실제 내용은 다음과 같다.

| 쌍 | 청크 | 비율 |
|---|---|---:|
| `ADR-020: Single 구성 고가용성` ↔ `고가용성(HA) 전략` | 8·8 | 1.00 |
| `PRD` ↔ `UI 디자인 가이드` | 4·8 | 1.00 |
| `ADR-009: 폴링이 주 경로` ↔ `7. LISTEN / NOTIFY 실측` | 4·6 | 1.00 |
| `ADR-010: Primary 강제` ↔ `6. 읽기/쓰기 분리와 복제 지연` | 4·4 | 1.00 |

전부 **주제가 관련될 뿐 같은 내용이 아니다.** 원인은 짧은 문서가 아니라 판정 구조다 —
비율의 분모가 자기 청크 수이고 `nearest_chunks`가 **순위 기반**(절대 거리 임계 없음)이라,
코퍼스 안에 최근접 이웃은 언제나 존재한다. 주제가 가까운 두 문서는 자기 대목 전부가 상대에서
최근접을 찾아 비율이 1.0에 붙는다. ADR-029의 하한 `matched_chunks >= 2`는 1청크 문서만 막고
이 자리는 막지 못한다.

**결론.** `overlaps`는 "거의 같은 내용"의 지표가 아니라 **"여러 대목에서 서로 가장 가깝다"**는
강한 관련성 신호다. 신호 자체는 잘 작동하므로 임계를 바꾸지 않고, **이름과 화면 문구를 신호에
맞췄다**(`UI_GUIDE.md` 관계 어휘표, 진단 화면). 확정 중복은 `content_hash`가 일치하는
`identical`뿐이다. 절대 거리 임계 도입은 #38이 컷오프를 순위로 바꾼 결정을 되돌리는 일이라
이 자리에서 다루지 않았고, **미해결 항목으로 남긴다.**
...
| `MIN_MATCHED_CHUNKS` | **3 (양쪽)** | 매칭 청크 절대 수 하한 **[2026-08-11 추가 · 2026-09-16 양쪽 3으로, §16]** |
...
> **[개정 2026-09-16, #94]** 하한을 **양쪽 3**으로 올렸다. 한쪽 2는 계산 주체가 긴 쪽일 때 1청크
> 이웃을 `1/1`로 통과시켰고, 2청크에서는 비율이 0·0.5·1.0뿐이라 "둘 다 겹쳤다"에 판별력이 없었다 —
> 시연 코퍼스 실측과 규칙 후보 비교는 §16 「리뷰 뒤 실측」에 있다.
...
### Step 1 — 조회 시점 제목 resolve를 순회에 포함

m9 step 0 이후 로컬 seed(문서 63건, `document_edges` 1,252행, `document_links` 128행)에서
깊이 2의 문서 id 순회를 5회씩 측정했다. 두 쿼리 모두 권한 필터를 재귀항 안에 두었고,
resolve 결합형은 `document_links.target_title = documents.title` 조인을 CTE에서 수행해 열람 가능한
동명 문서를 모두 인접 노드로 만들었다.

| 순회 경로 | 중앙 실행 시간 | 5회 범위 | shared hit 합 중앙 |
|---|---:|---:|---:|
| 저장된 `document_edges`만 | 0.952 ms | 0.876~3.350 ms | 5,212 |
| `document_edges` + 조회 시점 제목 resolve | **1.220 ms** | 1.211~1.322 ms | 5,568 |

워밍업이 섞인 첫 edge 실행을 포함해도 결합형은 2 ms 아래였고, 중앙값 증가는 0.268 ms였다.
`EXPLAIN (ANALYZE, BUFFERS)`에서 제목 resolve는 `document_links`와 열람 가능한 `documents`의
**Hash Join**이었고 두 입력은 각각 Seq Scan이었다. 현재 입력이 128행·63행이라 이 계획이 싸며,
resolve 뒤 목적지 권한 확인은 `documents_pkey` Index Scan을 사용했다. 즉 제목 조인 자체는 현재
인덱스를 타지 않으며 아래 재측정 조건을 남긴다.
따라서 3순위 경로인 위키링크를 순회에서 빼야 할 비용은 현재 seed에서 관측되지 않아 `refers`로
포함한다. 링크 대상 권한은 제목 조인의 `ON` 조건에서 적용하고 재귀항에서도 다시 확인하므로,
볼 수 없는 대상은 인접 노드가 되지 않아 순회가 통과하지 못한다. 동명 대상은 1:N으로 퍼지고,
깨진 링크는 조인 결과가 없어 순회 대상이 되지 않는다.

이 값은 63문서 seed의 계획 검증이며 대규모 그래프의 지연 근거가 아니다. 문서 제목은 현재 별도
인덱스가 없으므로 문서 수가 커지면 resolve 조인 비용을 다시 측정해야 한다.

### Step 3 — `pg_trgm` + 벡터 RRF 비교와 미채택

> 🔴 **아래 1차 측정의 표는 근거로 쓸 수 없다** — 키워드 순위 함수를 잘못 골라 그 열이
> 잡음이다. **이 절 끝의 「정정」 블록을 먼저 읽어라.** 미채택이라는 결론은 유지되지만
> 이유가 다르다.

m9 step 0 이후 같은 로컬 seed를 실 `BAAI/bge-m3` 상태로 측정했다. 문서 **63건**, 청크
**299행**이며 `count(DISTINCT embedding::text) = 299`로 벡터 퇴화가 없었다. 후보 수는 각
경로 25개, 문서당 1건 축소는 RRF 융합 뒤에 적용했다. 키워드 경로는
`content ILIKE '%' || query || '%'`로 정확·부분 일치 후보를 잡고
`similarity(content, query)`로 순위를 매겼다. 두 경로에는 같은 열람 술어를 넣었다.

아래는 10개 질의의 각 방식 **1위 문서**다. 제목은 표 너비를 위해 줄였다.
**「키워드만」·「융합」·「판정」 세 열은 무효다**(아래 정정 블록) — 「벡터만」 열만 유효하다.

| 질의 | 벡터만 | ~~키워드만~~ | ~~융합 (`K=60`)~~ | ~~판정~~ |
|---|---|---|---|---|
| OpenSQL | 구성요소 | 고가용성 전략 | 고가용성 전략 | 벡터보다 나빠짐 |
| 정합성 | 시스템 개요 | 설계 영향 요약 | 시스템 개요 | 유지 |
| 임베딩 | 프로젝트 개요 | 워커 ADR | 시스템 개요 | 개선 근거 불충분 |
| 고가용성 | 고가용성 전략 | 고가용성 전략 | 고가용성 전략 | 유지 |
| 위키링크 | UI 가이드 | 프로젝트 규칙 | 프로젝트 규칙 | 개선 근거 불충분 |
| 권한 | 태그 추천 ADR | 태그 추천 ADR | 태그 추천 ADR | 1위는 유지, 로그인 ADR은 2→3위 |
| OpenProxy | **공식 접속 방식** | 고가용성 전략 | 고가용성 전략 | **정답성 높은 1위가 5위로 하락** |
| 벡터 인덱스 | 벡터 인덱스 | 관련 문서·추천 | 벡터 인덱스 | 유지 |
| 재임베딩 | 관계 저장 ADR | API 설계 | **API 설계** | 개선 |
| 태그 추천 | **태그 추천 ADR** | 관계 실측 | 관련 문서·추천 | ADR이 1→4위로 하락 |

~~RRF가 확실히 고친 것은 `재임베딩` 한 건이었다. 반대로 `OpenProxy`·`OpenSQL`·`태그 추천`에서
벡터의 정답성 높은 상위 문서를 내렸고, 나머지는 유지이거나 어느 쪽이 더 낫다고 판정할 근거가
없었다.~~ **(무효 — 아래 정정)** 채택 조건을 충족하지 못한 것은 맞으나, 위 문장은 잡음
랭킹을 관찰한 결과라 이유가 될 수 없다.

`K=10·30·60·100`도 비교했다. 10개 중 **9개 질의의 1위가 네 값에서 동일**해, 관례값 60을
조정해도 위 결론은 바뀌지 않았다. 중앙 실행 시간은 벡터만 **1.68 ms**, 키워드만 **5.36 ms**,
융합(`K=60`) **5.93 ms**였고 융합 범위는 1.82~15.54 ms였다. 이 값은 63문서 seed의 상대 비교일
뿐 대규모 성능 근거가 아니다.

측정용 `gin (content gin_trgm_ops)` 인덱스는 **1,384 kB**였다. `정합성` 계획은
`Bitmap Index Scan on idx_chunks_content_trgm` → `Bitmap Heap Scan`으로 실제 GIN을 사용했다.
즉 미채택 이유는 인덱스를 못 타거나 느려서가 아니라 **랭킹 품질이 일관되게 좋아지지 않았기
때문**이다.

**결정: RRF와 trigram 인덱스를 제품에 채택하지 않는다.** 측정용 검색 SQL과 인덱스 마이그레이션은
회수했고 기존 벡터 진입점과 그래프 확장을 그대로 유지했다. `pg_trgm` 확장 자체는 위키링크 이전의
관계 방향 실측 재현 근거로 005 마이그레이션에 남지만 검색 경로에서는 사용하지 않는다. #12 본문은
폐기된 FTS 원안이 다시 근거로 쓰이지 않도록 `pg_trgm` 실험안과 이 측정 조건으로 정정했다.

#### ⚠️ 정정 (2026-08-11, 같은 날 재측정) — 결론은 옳고 위 표의 근거는 무효다

**위 표의 「키워드만」·「융합」·「판정」 세 열을 근거로 인용하지 마라.** 키워드 순위를
`similarity(content, query)`로 매긴 것이 잘못이었다. 이 함수는 **두 문자열 전체**의 trigram
자카드 유사도라 청크가 길수록 분모가 커져 0에 수렴한다 — 질의어가 정확히 들어 있어도 점수가
오르지 않는다. 실제 값은 `OpenProxy` 0.065 · `OpenSQL` 0.052 · `태그 추천` 0.026 ·
`정합성` 0.021 · `재임베딩` 0.013으로 **전부 잡음 수준**이고, 그 순위는 사실상 **청크 길이
순**이었다. 위 표의 「키워드만」 열은 이 잡음 순위를 그대로 옮긴 것이다.

`word_similarity(query, content)`로 바꾸면 후보는 제대로 잡힌다(같은 다섯 질의에서 1위가
전부 점수 **1.000**, 질의어를 포함한 문서). #29가 근거로 삼은 것도, m7 step 5의 `broader`
판정이 실제로 쓰는 것도 이쪽이다.

**그런데 바꿔도 융합은 나아지지 않았고, 그 이유가 더 근본적이다 — `word_similarity`는
랭킹 함수가 아니라 이진 필터다.**

| 질의 | 후보 수 | 점수 1.000 동점 | 서로 다른 점수 |
|---|---|---|---|
| `OpenSQL` | 125 | **109** | 4 |
| `OpenProxy` | 101 | **77** | **2** |
| `임베딩` | 72 | 59 | 2 |
...
질의어가 content 안에 온전히 있으면 무조건 1.000이므로, 후보의 대부분이 동점이 되고 순위는
보조 정렬(`c.id`)이 정한다 — **사실상 무작위다.** RRF는 순위를 입력으로 받는 알고리즘이라,
한쪽 입력이 무작위면 융합은 정보를 더하는 게 아니라 **잡음을 섞는 일**이 된다. 재측정에서
`OpenProxy`의 벡터 1위(`4. 공식 접속 방식`)가 융합에서 `ADR-020`으로 밀린 것이 그 결과다.

**따라서 미채택 결론은 유지된다. 근거만 교체한다:**

- ~~융합이 벡터의 정답성 높은 상위 문서를 낮췄다~~ (틀린 키워드 랭킹 위에서 나온 관찰)
- **trigram 경로가 순위를 만들지 못한다.** 포함 여부는 판정하지만 *얼마나 잘 맞는지*는
  구분하지 않는다. 순위 없는 입력으로는 RRF가 성립하지 않는다

빈도·문서 길이 정규화(BM25 계열)가 있어야 순위가 생기는데 trigram으로는 만들 수 없고,
번들에 한국어 형태소 분석기가 없어 `tsvector` 경로는 #29가 이미 배제했다. **한국어에서
키워드 랭킹을 세우려면 형태소 분석기가 먼저다** — 그것이 이 결정의 진짜 제약이다.

> **일반화**: 순위 기반 판정은 분자를 포화시킨다. m7의 겹침 판정(§14, `overlap_ratio`)에서
> 한 번 밟은 함정을 검색 랭킹에서 다시 밟았다. **점수 분포를 먼저 보지 않고 순위를 쓰면,
> 동점 덩어리가 보조 정렬로 갈려 무작위가 된다.**

#### 10질의 3방식 비교표를 다시 만들지 않는다 (step3 AC 7 처리)

정의서 `phases/m9-wikilink-rrf/step3.md:84`는 질의 10개를 「벡터만 / 키워드만 / 융합」 세
방식으로 비교하라고 요구했고 AC 7이 그 표를 가리킨다. **이 표는 `word_similarity`로 다시
측정해도 근거가 되지 않으므로 만들지 않는다.**

바로 위 동점 분포가 그 이유다. 「키워드만」 열은 1위 문서 하나를 적는 칸인데, 후보 125개 중
109개가 1.000 동점이면 그 1위는 보조 정렬이 뽑은 것이다. 표를 채우면 **무작위로 뽑힌 문서를
「키워드 1위」라고 적고, 그 위에서 계산한 「융합」·「판정」 열을 근거로 남기게 된다** — 1차
측정이 무효가 된 것과 같은 구조의 잘못을 함수만 바꿔 반복하는 셈이다.

대신 남기는 것은 **동점 분포 표 자체**다. 「키워드 순위가 성립하지 않는다」는 것이 측정 결과이며,
그것으로 RRF 미채택이 결정된다. 순위가 없는 입력을 억지로 표에 적는 것보다, 순위를 세울 수 없다는
사실을 적는 편이 정확하다. 유효한 비교를 하려면 한국어 형태소 분석기가 먼저이며 (위 문단), 그때는
질의 선정부터 다시 해야 한다.

---

## 15. 덩어리 밀도와 이웃 개수 실측 [실측 2026-08-27]

「관계 지도」의 덩어리가 부서와 어긋나는 원인을 찾고, 손잡이가 무엇인지 가린 측정이다.
결론부터: **거리 임계는 효과가 없고, 손잡이는 이웃 개수(`NEIGHBOR_N`)와 Louvain
`resolution`이다.** 다만 이 코퍼스가 66문서로 작아서 드러나는 성질일 수 있어, 파라미터를
바꾸는 결정의 근거로 삼기 전에 규모가 다른 코퍼스에서 다시 재야 한다.

### 측정 환경과 재현 방법

실 OpenSQL VM(PostgreSQL 17.8 + pgvector 0.8.1, OpenProxy:6432)에 `scripts/demo_corpus/`
64건과 업로드 2건, 합계 **문서 66 · 청크 66**을 실 `BAAI/bge-m3`로 적재한 상태에서 측정했다.
문서당 청크가 1개이므로 청크 그래프와 문서 그래프가 같다.

트리거(008)를 고치지 않고 시뮬레이션했다. `document_chunks.embedding`을 전부 읽어 정규화한
뒤 코사인 거리 행렬을 만들고, **트리거와 같은 규칙**(청크마다 다른 문서의 최근접 N개)으로
문서쌍을 만든다. 그 문서쌍 집합을 `app.services.clusters._assign_communities`에 **그대로**
넘긴다 — 군집화 코드는 실제 구현을 태운다.

**순도**는 각 덩어리에서 최다 부서가 차지하는 비율을 덩어리 크기로 가중 평균한 값이다.
부서는 `tags[0]`(경영지원·고객지원·물류·보안)이다. 무작위 배정의 기대값은 약 0.25다.

### (a) 거리 임계는 자를 것이 없다

문서쌍 435개의 거리 분포가 좁다 — 10% 0.287 · 25% 0.317 · 50% 0.353 · 75% 0.378 ·
**최대 0.447**.

| 임계 | 남은 쌍 | 덩어리 | 순도 |
|---|---|---|---|
| 없음 (현재) | 435 | 4 | 0.70 |
| `dist < 0.45` | 435 | 4 | 0.70 |
| `dist < 0.42` | 419 | 5 | 0.65 |
| `dist < 0.40` | 393 | 5 | 0.68 |
| `dist < 0.38` | 334 | 5 | 0.58 |
| `dist < 0.35` | 204 | 6 | 0.64 |

0.45 이상은 한 쌍도 자르지 못한다. 더 조이면 쌍이 절반 이하로 줄어드는데도 순도가 **떨어진다**
— 같은 부서와 다른 부서의 거리 분포가 겹쳐 있어 임계로는 둘을 가르지 못한다는 뜻이다.

### (b) 손잡이는 이웃 개수다

| 이웃 | resolution | 문서쌍 | 그래프 밀도 | 덩어리 | 순도 |
|---|---|---|---|---|---|
| **10** | **1.0** *(현재)* | 435 | 20.3% | 4 | **0.70** |
| 10 | 1.3 | 435 | 20.3% | 6 | 0.77 |
| 8 | 1.0 | 347 | 16.2% | 5 | 0.79 |
| 6 | 1.3 | 267 | 12.4% | 6 | 0.83 |
| **5** | **1.6** | 227 | 10.6% | 6 | **0.86** |
| 4 | 1.6 | 187 | 8.7% | 8 | 0.76 |
| 3 | 1.3 | 140 | 6.5% | 8 | 0.74 |
| 2 | 1.6 | 94 | 4.4% | 10 | 0.71 |

어느 조합에서도 미분류(관계가 없어 어디에도 못 붙는 문서)는 0건이었다. 최고점인
**이웃 5 · resolution 1.6**에서 덩어리가 이렇게 나온다.

```
물류(13)         물류 11 · 고객지원 1 · 경영지원 1
경영지원(13)     경영지원 13
고객지원(11)     고객지원 10 · 경영지원 1
경영지원 (2)(11) 경영지원 10 · 보안 1
보안(11)         보안 10 · 태그없음 1
물류 (2)(7)      섞임
```

여섯 중 다섯이 한 부서로 정렬된다. 경영지원이 둘로 갈린 것은 **인사와 재무**이며, 이름이 둘 다
「경영지원」이 되는 것은 라벨이 `tags[0]` 최빈값이기 때문이다 — 이슈 #89 관찰 1과 같은 자리다.

### (c) 저장된 그래프는 임베딩 순서에 의존한다

같은 규칙으로 계산한 대칭 그래프는 **435쌍**인데 VM에 실제로 저장된 것은 **591쌍**이다.
008 트리거가 문서마다 `DELETE`(자기가 걸린 모든 행) → `INSERT`(자기 기준 양방향)를 하므로,
어떤 쌍이 남는지는 두 문서 중 **나중에 처리된 쪽의 top-N**이 정한다. ADR-029가 "정확한 역방향은
문서 수에 비례해 비싸므로 택하지 않았다"고 트레이드오프로 적어 둔 결과가 여기서 관측된다.

같은 이유로 덩어리 배치가 흔들린다. 문서 한 건의 문장 하나를 고쳐 재임베딩했더니 관계가
605쌍 → 591쌍으로 바뀌면서 **덩어리 4개가 3개로 합쳐졌다**(고객지원이 물류에 흡수).

### 하지 않은 것과 그 이유

`NEIGHBOR_N`은 008 트리거 안에 있고 관련 문서·검색의 관계 확장·태그 추천이 모두 그 값을 쓴다.
§14가 N=10을 정당화한 근거이기도 하다. 이 측정만으로 바꾸지 않는다.

더 큰 이유는 **규모 의존이 걷히지 않았다는 것**이다. 66문서에서 이웃 10개는 전체의 15%지만
1,000건이면 1%다. 밀도 20.3%는 파라미터가 틀려서가 아니라 코퍼스가 작아서 나온 값일 수 있다.
규모가 다른 코퍼스에서 같은 스윕을 돌리기 전에는 "N=10이 과하다"고 말할 수 없다.

> **§16으로 답이 났다 (2026-09-16).** 규모가 다른 두 코퍼스(184문서·104문서)에서 같은 스윕을
> 돌렸다 — **N=10은 유지**하고, 손잡이는 청크당 이웃 개수가 아니라 **문서당 이웃 상한**이었다.

---

## 16. 관계 판정 규칙 재설계 실측 [실측 2026-09-16]

#93 실코퍼스 검증이 남긴 관계 결함(오탐 `overlaps` · 재실행마다 흔들리는 그래프 · 트리거 40초)을
고치기 위해 규칙 후보를 실 임베딩 위에서 비교한 측정이다(#94). 결론부터: **단방향 저장 + 양쪽
비율 + 문서당 이웃 5건**을 채택했고, `NEIGHBOR_N=10`은 유지하며, 순서 의존은 규칙이 아니라
**전량 재계산**(`openarchive rebuild-edges`)으로 푼다. 결정은 ADR-029 개정 블록·결정 6에 있다.

### 측정 환경과 재현 방법

**코퍼스 둘.** A = 로컬 `pgvector/pgvector:pg17` 컨테이너, 문서 **184 · 청크 1,080**(저장소 `docs/`와
...
문서 id로 푼다(동명 문서는 전부 정답). `search_documents`를 화면과 같은 `k=10`으로 부르고 결과를
문서 단위로 접어 순위로 쓴다. `fake` 프로바이더면 실행을 거부한다.
...
실 임베딩을 덤프해 청크 kNN을 파이썬으로 계산하고, 규칙(비율·상한·저장 방향)을 바꿔 가며 문서쌍
집합을 만든 뒤 실제 구현인 `app.services.clusters._assign_communities`에 **그대로** 넘겼다 — §15와
같은 방법이다. 트리거는 처리 시점까지의 문서만 후보로 보므로, 시뮬레이터도 적재 순서대로 **"처리
시점까지의 문서만 후보"**로 재현했다. 이 재현이 실제 저장 edge와 일치하는지 먼저 확인했다 —
**A 자카드 1.000(kind까지 일치) · C 0.989**. 재현이 맞지 않으면 아래 표 전체가 무의미하므로 이것이
첫 관문이다.

**순도**는 §15와 같이 덩어리별 최다 분류 비율의 크기 가중 평균이다(C는 기능 분류 9종, A는 디렉토리
23종). **overlaps 정밀도·재현율**의 정답은 C의 판본 24쌍이다. **related 동일분류**는 `related`
쌍 중 같은 분류에 속한 비율, **밀도**는 문서쌍 수 / 가능한 쌍 수, **재실행 자카드**는 같은 규칙을
두 번 돌린 저장 edge 집합의 자카드다.

### 기준선 — 변경 전 검색 품질

규칙을 바꾸기 전에 검색 평가셋으로 기준선을 남겼다. **변경 후 수치는 이 phase 뒤에 다시 재어
이슈 #94에 적는다** — 여기 적힌 것은 "변경 전"뿐이다.
...
C의 R@1이 낮은 것은 정답이 판본 포함 2~3개라 상한이 1/n인 탓이다.
...
- **양쪽 비율**이 `overlaps` 오탐을 143 → 59로 줄이고 판본 24/24는 그대로 잡는다(정밀도 0.168 → 0.407).
  남는 35쌍은 위원회 운영규칙 등 **규정 상용구가 유사한 쌍** — 규칙으로 더 자를 수 없어 **임베딩
  모델의 한계로 기록**한다. 비율 0.9는 판본 2쌍을 놓친다(재현율 0.917).
- **문서당 상한**이 순도를 움직인다. C 0.385 → 0.577, A 0.505 → 0.543이 cap 5에서 최고이고 cap 3은
  다시 내려간다. `related` 동일분류도 0.198 → 0.390(C)·0.234 → 0.390(A)이다.
- **`NEIGHBOR_N=10` 유지.** `N=5 + cap 5`는 순도 0.567로 cap 5와 ±0.01 차이인데 재현율이 0.958로
  떨어진다. §15가 이웃 개수를 손잡이로 본 것은 문서당 상한이 없던 그래프의 관측이었다.
- **단방향 저장**은 재실행 자카드를 0.971 → 0.990으로 올린다. 008의 `DELETE both`가 재실행만으로
  순도를 0.385 → 0.375로 떨어뜨리던 것이 멈춘다.

### 순서 의존의 본체는 "처리 시점까지의 문서만 후보"다

"순차 적재" 결과와 "전체 코퍼스 기준" 결과의 문서쌍 일치(j_full)는 cap 5에서 **C 0.371 · A 0.477**이고,
적재 순서를 섞으면 두 순서의 결과가 **0.310 · 0.369**만 겹친다. 순도는 둔감하지만(0.577 vs 0.587) 쌍은
흔들린다. 단방향 저장으로도 이것은 남는다 — 먼저 들어온 문서가 나중 문서를 발견할 기회가 없기
때문이다. 이것이 `rebuild-edges`(ADR-029 결정 6)의 근거다.

시연 코퍼스(64문서·1청크·실 BGE-M3)에서 같은 것을 봤다: 008 순차 0.844 → 재실행 3건 뒤 **0.688**
(§15 (c) 재현), 전체 기준 0.734. cap 5 순차 0.609, 전체 기준 **0.750**.

### 트리거 비용 — 플래너가 HNSW를 고르지 않았다 (#93 P1)

실 VM에서 청크 하나의 이웃 프로브가 **265 ms**였고 `enable_seqscan = off`에서 **14 ms**였다. 트리거
전체로는 **10청크 4.6 s → 0.2 s · 159청크 40 s → 2.7 s**다. `random_page_cost = 1.1`(§12 16번)로는
부족했다 — 1만 청크 미만에서는 여전히 Seq Scan을 고른다.

**`SET LOCAL`로는 안 됐다.** OpenProxy 풀 백엔드에 PL/pgSQL **generic plan**이 남아 있어 이전 설정으로
잡힌 계획을 재사용했고, `DISCARD PLANS` 뒤에야 `enable_seqscan = off`가 먹었다. `RESET ALL`은 플랜
캐시를 비우지 않는다 — §5-2의 임시 테이블·`LISTEN` 누수와 같은 계열이다. 그래서 세 설정을
**함수 정의의 `SET`**에 둔다(`014_edges_triggers.sql`). 재현할 때는 새 세션이 아니라 **같은 세션에서
6회 이상 실행한 뒤에도** 인덱스를 타는지 봐야 한다 — PL/pgSQL은 5회까지 custom plan을 쓰다가 그
뒤 generic plan으로 넘어간다.

> 잡 시간으로 트리거 비용을 추정하면 10배 넘게 틀린다. `embedding_jobs.finished_at`이 `now()`(트랜잭션
> 시작 시각)였기 때문이며, `clock_timestamp()`로 바꿨다(m13 step 3).

### 리뷰 뒤 실측 — 시연 코퍼스에서 대목 수 하한의 구멍 (2026-09-16)

phase는 시연 코퍼스의 실 BGE-M3 확인을 유예했다(step 6). 리뷰 뒤 스크래치 DB에 `seed_demo.py`를 실
프로바이더로 돌려 확인하니 **`overlaps` 44건**이었다(문서 64·청크 77·2청크 13건, 기대 0~1건). 39건이
"2청크 문서 → 1청크 이웃"이고 부서 간 쌍이 14건(「냉장·냉동 보관 온도 기준 ↔ 정보보안 사고 대응 절차」
1.00 등). 원인: `matched_src ≥ 2`가 계산 주체 쪽에만 걸려 상대가 1청크면 `1/1`로 통과하고, 77청크에서
top-10은 전체의 13%라 같은 주제 1청크 문서는 거의 다 걸린다. 위 「규칙 후보 비교」의 시뮬레이터로
후보를 시연 코퍼스(스크래치 DB 직접 읽기)와 C 덤프 양쪽에서 비교했다 — 전체 기준(순서 무관), cap 5.

| 규칙 | 시연 overlaps | 시연 같은부서 비율 | C overlaps | C 정밀도 | C 재현율(24쌍) |
|---|---|---|---|---|---|
| 014 초안 (`src ≥ 2`) | 44 | 0.68 | 28 | 0.857 | 1.000 |
| 양쪽 ≥ 2 | 13 | 0.46 | 28 | 0.857 | 1.000 |
| `src ≥ 3` | 0 | — | 28 | 0.857 | 1.000 |
| **양쪽 ≥ 3 (채택)** | **0** | — | 27 | **0.889** | **1.000** |
| 양쪽 ≥ 2 + `min_dist ≤ 0.10` | 0 | — | 25 | 0.920 | 0.958 |
| 양쪽 ≥ 2 + `min_dist ≤ 0.05` | 0 | — | 23 | 0.957 | 0.917 |

- 정답 판본 쌍의 `min_dist`는 최소 0.000·중앙 0.003·**최대 0.145** — 거리 임계 0.10은 1쌍, 0.05는 2쌍을
  잃는다. 이슈가 예상한 대로 거리로는 못 가른다.
- 순도(시연 0.797·C 0.587)·쌍 수·degree는 후보 간 동일하다. cap 5가 고른 이웃 집합은 같고 라벨만 바뀐다.
- C의 전체 기준 정밀도(0.857)가 위 표의 순차 적재 값(0.407)보다 높은 것은 규칙이 아니라 **기준의 차이**다
  — 순차 적재는 먼저 들어온 문서가 나중 문서를 보지 못해 오탐이 남고, 전량 재계산(결정 6) 뒤에는 그것이
  사라진다.
- 시연 코퍼스의 덩어리는 6개·가중 순도 0.80(8/27 기준선 4개·≈0.84)이며 「배송」7문서 순도 0.43이 섞였다.
  이것은 하한과 무관하게 cap 5 이웃 집합이 정한 결과라 **#94 범위 밖**으로 남긴다 — 시연 품질 문제로
  코퍼스 손질 또는 수용을 따로 판단한다.

### 하지 않은 것과 그 이유

- **`mutual`(양쪽 다 계산됐을 때만 `related`)** — 순차 적재에서 먼저 들어온 문서는 나중 문서를 계산할
  기회가 없어 `related`가 **0건**이 된다. 전멸.
- **트리거 안 역방향 재계산** — 새 문서가 들어올 때 이웃들의 `src` 행도 다시 계산한다. 비용 5배이고
  kNN 비대칭이라 불완전하다. 전량 재계산은 한 번에 완전한 결과를 준다.
- **절대 거리 임계** — §15 (a)에서 자를 것이 없었고, ADR-029 결정 4의 순위 컷오프를 되돌리는 일이다.
- **변경 후 검색 평가** — 이 측정에서 하지 않았다. phase 뒤 같은 평가셋으로 재어 #94에 적는다.
