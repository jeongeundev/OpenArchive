...
| 17 | **JOIN이 원인이라는 결론의 변수 분리** | `cand`에 **JOIN만 두고 권한 필터는 빼서** `EXPLAIN` 1회 | 12번 표의 39ms(JOIN 없음·무필터)와 255ms(JOIN+권한 필터)는 **두 변수가 동시에 바뀐다.** 원인을 JOIN으로 특정한 근거가 실측으로 분리되지 않았다. 만약 진짜 원인이 필터 쪽이면 ADR-018 개정의 방향(권한 필터를 뒤로)이 무의미해진다 — 다만 신 설계가 실제로 빨라졌으므로 결론이 뒤집힐 가능성은 낮고, **근거 보강 성격**이다 |
