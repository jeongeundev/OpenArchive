...
#
# 여유가 300초인 이유: lifespan에 **모델 예열**이 들어 있다 (ADR-003 보강). 가중치가
# 캐시된 호스트는 12~13초지만, 캐시가 없는 첫 배포는 BGE-M3를 내려받는 시간이 더해진다
# (모델 캐시 4.3GB). 60초로는 첫 배포가 실패한다 — 예전에 첫 검색이 떠안던 지연이
# 기동으로 옮겨졌으므로 이 게이트도 그만큼 넓어야 한다.
api_ready=""
...
[[ -n "$api_ready" ]] \
  || fail "API·프론트가 300초 안에 응답하지 않았다(마지막 상태코드: ${code:-없음}). 첫 배포라면 모델 내려받기가 끝나지 않았을 수 있다. ~/api.log ~/web.log 를 확인한다"
